...
const STATUS_CLS: Record<string, string> = {
  ...
  angepasst: "bg-orange-100 text-orange-700",
...
export function getAllParticipants(positions: Position[]): string[] {
  ...
  for (const pos of positions) {
    for (const a of pos.assigned) {
      const match = a.match(/(.+?)\s*\((\d+)\s*[x×]\s*\)/i);
...
export function computeBetrag(positions: Position[], name: string): number {
  ...
  return positions.reduce((acc, pos) => {
    ...
    const mine = pos.assigned.some(a => {
      const match = a.match(/(.+?)\s*\((\d+)\s*[x×]\s*\)/i);
      const cleanName = match ? match[1]!.trim() : a;
      return cleanName.toLowerCase() === lc;
    });
...
export function ShareRequestCard({ request: r, token, personName }: ShareRequestCardProps) {
  ...
  const adjustMutation = useMutation({
    mutationFn: ({ betrag, positions }: { betrag: number; positions: any[] }) =>
      shareLinksApi.adjustSplit(token, r.id, { betrag, positions }),
    onSuccess: () => {
      setIsEditing(false);
  ...
  const isAngepasst = r.status === "angepasst";
  const isBusy = statusMutation.isPending;
  const hasPositions = r.positions && r.positions.length > 0;
  const canEdit = (isPending || isAccepted || isAngepasst) && !!hasPositions;

  const viewParticipants = hasPositions ? getAllParticipants(r.positions!) : [];

  return (
    <div className="rounded-xl border bg-card p-4 sm:p-5 flex flex-col gap-4 transition-all shadow-sm hover:shadow-md border-border">
      {/* Header */}
        ...
        <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
          ...
          <div className="flex items-center gap-1.5 flex-wrap justify-end">
            {r.adjustedByRecipient === 1 && (
              <span className="inline-flex px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider bg-orange-100 text-orange-700 animate-pulse">
                Angepasst (Freigabe ausstehend)
              </span>
            )}
            {r.adjustedByRecipient === 2 && (
              <span className="inline-flex px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider bg-emerald-100 text-emerald-700">
      ...
      {/* ── POSITIONS – EDIT MODE (inline modal content) ── */}
      {hasPositions && isEditing && (
        <ReceiptDetailModal
          ...
          receipt={{
            id: r.id,
            user_id: "",
            datum: r.datum || new Date().toISOString().slice(0, 10),
            haendler: r.haendler,
            betrag: r.positions!.reduce((sum: number, p: Position) => sum + p.amount, 0),
            mwst: 0,
            trinkgeld: 0,
            ...
            zahlungsmethode: "",
            rechnungsnummer: "",
            driveLink: r.hasReceipt ? "mock" : "",
            eingabe_typ: "foto",
            erstellt_am: "",
          } as any}
          initialTab="aufteilen"
          ...
          isRecipient
          recipientRequest={r}
          customPreviewUrl={r.hasReceipt ? shareLinksApi.receiptPreviewUrl(token, r.id) : undefined}
          onAdjustSplit={async (betrag, positions) => {
            await adjustMutation.mutateAsync({ betrag, positions });
          }}
        />
      )}
                ...
                {pos.assigned.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-0.5">
                    {pos.assigned.map((name, nIdx) => {
                      const match = name.match(/(.+?)\s*\((\d+)\s*[x×]\s*\)/i);
                      const cleanName = match ? match[1]!.trim() : name;
                      const isMe = cleanName.toLowerCase() === personName.toLowerCase();
                      return (
                        <span
                          key={nIdx}
